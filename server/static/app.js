...
function connectButtonHandler(event) {
    ...
    if (!connected) {
        ...
        disconnect();
        button.innerHTML = 'Начать стрим';
        connected = false;
        shareScreen.innerHTML = 'Поделиться экраном';
...
function updateParticipantCount() {
    if (!connected)
        count.innerHTML = 'Нет подключения.';
    else
        count.innerHTML = (room.participants.size + 1) + ' online.';
};
...
function shareScreenHandler() {
    ...
    if (!screenTrack) {
        navigator.mediaDevices.getDisplayMedia().then(stream => {
            ...
            shareScreen.innerHTML = 'Прекратить экран';
        }).catch(() => {
